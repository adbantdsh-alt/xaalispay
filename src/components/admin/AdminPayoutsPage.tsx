...
export function AdminPayoutsPage() {
  ...
  const forceSucceedPayout = async (payoutId: string) => {
    if (!confirm("Marquer ce retrait comme réussi ? À faire uniquement si Bictorys confirme que le virement est arrivé.")) return;
    setForcingId(payoutId);
    setError("");
    const res = await apiFetch(`/api/admin/payouts/${payoutId}/force-succeed`, { method: "POST" });
    const data = await res.json();
    setForcingId(null);
    if (!res.ok) { showError(extractApiError(data, "Impossible de forcer le statut")); return; }
    await fetchPayouts(true);
    refreshOverview({ silent: true });
  };
